...
class SourceNetwork(nn.Module):
    def __init__(self, num_kernels, number_of_class=11, input_channel=1, kernel_size=(3, 5), dropout=0.2):  #0.5 originally
...
class TargetNetwork(nn.Module):
    def __init__(self, weight_pre_trained_convNet, num_kernels, number_of_class=11, input_channel=1, kernel_size=(3, 5),
                 dropout=0.5, size_image=(10, 150)):
        ...
        for i in range(num_levels):
            ...
            layers.append(TemporalBlock(in_channels, out_channels, kernel_size, stride=1, dilation=dilation_size,
                                        padding=(0, (kernel_size[1] - 1) * dilation_size), dropout=dropout))
            scale_layers.append(ScaleLayer(parameters_dimensions=(1, 1, size_image[0]-((self._kernel_sizes[0]-1)*(i+1)),
                                                                  150)))
        ...
        pre_trained_model.load_state_dict(weight_pre_trained_convNet)
    ...
    def forward(self, x, get_features=False, get_all_tasks_output=False, lambda_value=1., max_scale_value=5.):
