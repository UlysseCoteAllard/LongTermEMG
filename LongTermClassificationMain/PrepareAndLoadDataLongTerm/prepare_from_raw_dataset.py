...
def format_examples_and_timestamps(emg_examples, timestamps, window_size=150, size_non_overlap=50):
    examples_formatted, examples_formatted_timestamps = [], []
    ...
    return examples_formatted, examples_formatted_timestamps
...
def read_files_to_format_evaluation_session(path_folder_examples, window_size, size_non_overlap):
    ...
    for index_timestamp_example in range(len(timestamps_from_examples)):
        while True:
            ...
            #print("CURRENT TIME DIFFERENCE: ", current_time_difference)
            if current_time_arm >= timestamps_from_examples[index_timestamp_example]:
                #print("PREVIOUS: ", previous_time_difference, " CURRENT: ", current_time_difference)
        ...
        if current_time_arm > timestamps_from_examples[index_timestamp_example]:
            ...
            #print("time difference: ", current_time_arm-timestamps_from_examples[index_timestamp_example])
        else:
            time_difference = timestamps_from_examples[index_timestamp_example]-current_time_arm
            #print("time difference: ", timestamps_from_examples[index_timestamp_example]-current_time_arm)
...
def read_files_to_format_training_session(path_folder_examples, number_of_cycles, number_of_gestures, window_size,
                                          size_non_overlap):
...
def get_data_and_process_it_from_file(path, number_of_gestures=11, number_of_cycles=4, window_size=150,
                                      size_non_overlap=50):
...
def read_data_training(path, number_of_gestures=11, number_of_cycles=4, window_size=150, size_non_overlap=50):
    ...
    '''
    np.save("../Processed_datasets/LongTermDataset", dataset_dictionnary)
    np.save("../Processed_datasets/LongTermDatasetTrain", (list_dataset_train_emg, list_labels_train_emg))
    np.save("../Processed_datasets/LongTermDatetimes", (training_datetimes, evaluation_datetimes))
    '''


if __name__ == '__main__':
    read_data_training(path="../../datasets/longterm_dataset_3DC")
