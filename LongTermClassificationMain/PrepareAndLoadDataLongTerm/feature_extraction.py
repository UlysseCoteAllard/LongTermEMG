...
def getRootSquaredMoment2(vector, myLen):
    return np.sqrt(np.sum(np.square(vector)) / (myLen - 1))


def getRootSquaredMoment(vector):
    return np.sqrt(np.nansum(np.square(vector)))


def getSparseness(moment_zero, moment_two, moment_four):
    denominator = np.sqrt(np.abs((moment_zero - moment_two) * (moment_zero - moment_four)))
...
def getFeatures_for_tdpsd(vector):
    ...
    moment_two = np.power(getRootSquaredMoment2(first_derivative, len(vector)), lambda_variable) / lambda_variable
    moment_four = np.power(getRootSquaredMoment2(second_derivative, len(vector)), lambda_variable) / lambda_variable
...
def getTDPSDfeat_official(x, *slidingParams):
    ...
    if len(slidingParams) < 2:
        raise TypeError(
            'getTDPSDfeat expected winsize and wininc to be passed, got %d parameters instead' % len(slidingParams))
    if slidingParams:
    ...
    for i in range(numwin):
        ...
        ebp = features_for_TDPSD(curwin)
        efp = features_for_TDPSD(np.log(curwin ** 2 + np.spacing(1)))
...
def features_for_TDPSD(vector):
    ...
    m0 = np.sqrt(np.sum(vector ** 2, axis=0))[:, np.newaxis]
    m0 = m0 ** .1 / .1;

    # Prepare derivatives for higher order moments
    d1 = np.diff(vector, n=1, axis=0)
...
if __name__ == '__main__':
    ...
    print("Compare TDPSD implementations")
    own_implementation = get_TDPSD(a)
    Rami_implementation = getTDPSDfeat_official(np.swapaxes([a], 1, 0), *[len(a), 1])[0]
    print(own_implementation)
    print(Rami_implementation)
    print(np.allclose(own_implementation, Rami_implementation))

    own_implementation = get_TDPSD(b)
    Rami_implementation = getTDPSDfeat_official(np.swapaxes([b], 1, 0), *[len(b), 1])[0]
    print(own_implementation)
    print(Rami_implementation)
    print(np.allclose(own_implementation, Rami_implementation))

    own_implementation = get_TDPSD(c)
    Rami_implementation = getTDPSDfeat_official(np.swapaxes([c], 1, 0), *[len(c), 1])[0]
    print(own_implementation)
    print(Rami_implementation)
    print(np.allclose(own_implementation, Rami_implementation))
