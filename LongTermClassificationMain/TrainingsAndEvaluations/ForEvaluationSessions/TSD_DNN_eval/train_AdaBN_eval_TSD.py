...
from LongTermClassificationMain.Models.model_training import AdaBN_adaptation


def test_network_convNet_only_second_evaluation_session_for_each_training_session(
        examples_datasets, labels_datasets,
        dnn, num_neurons, number_of_classes=11, feature_vector_input_length=385,
        path_weights="Weights_TSD_eval/weights_THREE_CYCLES_TSD_ELEVEN_Gestures",
...
def run_AdaBN_evaluation_sessions(examples_datasets_evaluations, labels_datasets_evaluation, algo_name, num_neurons,
                                  path_weights_to_load_from, path_weights_SCADANN,
                                  batch_size=512, use_recalibration_data=False, number_of_classes=11,
    ...
    participants_evaluation_dataloader = load_dataloaders_test_sessions(
        ...
        labels_datasets_evaluation=labels_datasets_evaluation, batch_size=batch_size, shuffle=False, drop_last=True)
            ...
            if session_j % 2 == 0:
                ...
                if use_recalibration_data:
                    model, optimizer_classifier, scheduler, start_epoch = load_checkpoint(
                        model=model, optimizer=optimizer_classifier, scheduler=scheduler,
                ...
                best_state = AdaBN_adaptation(model=model, scheduler=scheduler,
                                              optimizer_classifier=optimizer_classifier,
                                              dataloader=participants_evaluation_dataloader[participant_i][session_j])
...
if __name__ == '__main__':
    path_weights = "../Weights/weights_THREE_CYCLES_SPECTROGRAMS_ELEVEN_Gestures"
    print(os.listdir("../../"))
    ...
    algo_name = "Three_cycles_AdaBN_EVAL_SESSIONS"

    # Using weights learned by normal classifier
    run_AdaBN_evaluation_sessions(examples_datasets_evaluations=examples_datasets_evaluation,
                                  labels_datasets_evaluation=labels_datasets_evaluation, num_neurons=num_neurons,
                                  number_of_classes=number_of_classes,
                                  path_weights_to_load_from=path_weights_fine_tuning,
                                  feature_vector_input_length=feature_vector_input_length,
                                  algo_name=algo_name, path_weights_SCADANN=path_weights_SCADANN,
                                  batch_size=512, use_recalibration_data=False)
    ...
    run_AdaBN_evaluation_sessions(examples_datasets_evaluations=examples_datasets_evaluation,
                                  ...
                                  feature_vector_input_length=feature_vector_input_length,
                                  algo_name=algo_name, path_weights_SCADANN=path_weights_SCADANN,
                                  batch_size=512, use_recalibration_data=True)
    ...
    test_network_convNet_only_second_evaluation_session_for_each_training_session(
        ...
        dnn=TSD_Network, feature_vector_input_length=feature_vector_input_length,
